...
data "terraform_remote_state" "dmz" {
  ...
  config = {
    bucket = "app-demo-3-tier-23nov-prod-state"
    key    = "prod/us-east-2/networking/dmz-access/terraform.tfstate"
    region = "us-east-2"
  }
}
...
data "terraform_remote_state" "data" {
  ...
  config = {
    bucket = "app-demo-3-tier-23nov-prod-state"
    key    = "prod/us-east-2/data-stores/mysql/terraform.tfstate"
    region = "us-east-2"
  }
}
...
module "webserver" {
  ...
  # Detailed monitoring is enabled in prod
  monitoring = true
